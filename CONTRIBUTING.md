...
|:---------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:------------|
| **Unreleased** | **TEITOK input is enriched again; nlp-enrich's page model read correctly** (#13 P5). A row without a `quality_score` (every TEITOK row, a CSV without the column) is *unknown* quality: the line filter's quality bands no longer turn it into "Trash" (`llm_client_shared.row_quality`, `llm_utils._row_quality`), so a `.teitok.xml` input reaches the model; `tests/conftest.py` `stub_llm` no longer patches the filter out. The GPU path (`llm_utils.py` `process_document` / `process_document_vllm`) passes the score as the 7th argument — every row used to raise and be swallowed. Re-vendored from atrium-nlp-enrich (its #38): `teitok_read.py` (line numbers per page; a sentence with a `<pb/>` inside gives one row per page part; `page_idx`/`page_label` keys), `flexiconv_convert.py` (exit 3 when flexiconv is missing, 1 when a conversion fails) and its test, plus the writer's sample `CTX000000002` as a pinned fixture. `xml_to_md.py --format layout`: page parts are separate lines with their own page's box, page labels head the sections, `DOC_META` says `origin=printspace` for PrintSpace-relative boxes. CONTRIBUTING no longer claims `llm_utils.py` is a verbatim copy. Since then (docs only): README *TEITOK input, in short* — where the format and its composition are described (atrium-nlp-enrich README), format 2's page model, and how to recognise files written with UDPipe-chunk pages (nlp-enrich v0.21.0 or earlier). **Defect V-1 fixed** (hub `docs/skos_strategy.md` §6, found by atrium-alto-postprocess#31): `json_to_md.DROP_CATEGORIES` is `atrium_vocab.UNTRUSTWORTHY_LINE_CATEGORIES` (Garbage, Inverted, Trash), so alto-postprocess's `Trash` lines no longer reach the model on the OCR path — a change to what the model is shown; pinned by `test_convert_drops_trash_lines_on_the_ocr_path`. | —           |
